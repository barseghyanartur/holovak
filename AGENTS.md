...
**Repository**: https://github.com/barseghyanartur/clipcut
...
```
ClipCut/
    ClipCutApp.swift          # @main entry point
    ...
    ClipCutViewModel.swift    # ObservableObject: state + export orchestration
    Info.plist                # Injects $(MARKETING_VERSION) as AppVersion
    Assets.xcassets/          # App icon

ClipCutTests/
    ClipCutTests.swift        # Unit tests for Segment and FFmpegRunner

ClipCutUITests/
    ClipCutUITests.swift      # Basic launch + UI state assertions

ClipCut.xcodeproj/            # Xcode project; do not hand-edit project.pbxproj
...
User drops/opens file
  └─ ContentView → ClipCutViewModel.loadFile(_:)
...
| `ClipCutViewModel` | `@MainActor class` | All mutable state; orchestrates export |
...
3. `ClipCutViewModel.canExport` requires `inputURL != nil` AND all segments
...
- `@MainActor` on `ClipCutViewModel` — all UI updates happen on the main thread.
...
- Do not hand-edit `ClipCut.xcodeproj/project.pbxproj`; use Xcode or
