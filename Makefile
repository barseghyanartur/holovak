...
## export: Extract the .app from the archive (Copy App method)
export:
	@test -d "$(ARCHIVE_PATH)" || \
...
## checksum: Compute and save the SHA-256 of the DMG (paste into the cask formula)
checksum:
	@test -f "$(DMG_PATH)" || \
